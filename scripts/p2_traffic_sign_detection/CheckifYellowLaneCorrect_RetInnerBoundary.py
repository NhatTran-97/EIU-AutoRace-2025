...
# import config
from Lane.utilities import findLineParameter,findlaneCurvature,Distance_,Cord_Sort
...
def GetYellowInnerEdge(OuterLanes,MidLane,OuterLane_Points):
	...
	# OuterLanes = OuterLanes.astype(np.uint8)
    # if OuterLanes.ndim == 3:
    #     OuterLanes = cv2.cvtColor(OuterLanes, cv2.COLOR_BGR2GRAY)
	...
	Outer_cnts, _ = cv2.findContours(OuterLanes, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
