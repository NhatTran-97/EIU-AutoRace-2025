...
def RetLargestContour(gray):
    ...
    # 2️⃣ Tìm contour đúng cú pháp cho OpenCV 4.x
    cnts, _ = cv2.findContours(bin_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # 3️⃣ Nếu không có contour nào, trả về mask rỗng
    if len(cnts) == 0:
        print("⚠️ RetLargestContour: Không tìm thấy contour nào.")
        return thresh, False

    # 4️⃣ Tìm contour có diện tích lớn nhất
    Max_Cntr_area = 0
    Max_Cntr_idx = -1
    for index, cnt in enumerate(cnts):
        if cnt is None or len(cnt) == 0:
            continue
        if cnt.dtype != np.int32 and cnt.dtype != np.float32:
    ...
    # 5️⃣ Nếu tìm được contour lớn nhất, vẽ nó lên mask
